...
LOG_FILE="/workspace/krea2test-background.log"
...
(
  ...
  echo "-------------------------------------------------------"
  echo "BACKGROUND WATCHER STARTED: KREA2 TEST CONFIG"
  echo "-------------------------------------------------------"
  ...
  if [ ! -d "$RES4LYF_NODE_DIR" ]; then
    # Sparse clone: RES4LYF is 216MB, of which 212MB (98%) is example_workflows/ and
    # workflows/ - example-image assets nothing at runtime imports. --filter=blob:none
    # means git never transfers blobs for the excluded dirs at all.
    echo "Cloning res4lyf (sparse, excluding example_workflows/ and workflows/)..."
    git clone --filter=blob:none --no-checkout --sparse "${CUSTOM_NODES[res4lyf]}" "$RES4LYF_NODE_DIR"
    git -C "$RES4LYF_NODE_DIR" sparse-checkout init --no-cone
    printf '/*\n!/example_workflows/\n!/workflows/\n' > "$RES4LYF_NODE_DIR/.git/info/sparse-checkout"
    git -C "$RES4LYF_NODE_DIR" checkout --quiet
  else
  ...
  echo "-------------------------------------------------------"
  echo "DOWNLOAD COMPLETE - KREA2 TEST INSTALLED"
  echo "-------------------------------------------------------"

) 2>&1 | tee -a "$LOG_FILE" >> /proc/1/fd/1 &

echo "krea2test.sh: background watcher started, main boot can continue"
echo "krea2test.sh: tail -f $LOG_FILE"
exit 0
