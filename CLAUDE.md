...
- **Do NOT add SageAttention to RunPod template scripts** — it requires building CUDA kernels from source (no prebuilt wheels exist for 2.x) which takes too long for a per-boot install. Bad experience for end users. SageAttention is only appropriate for persistent local/network drive setups where the build is a one-off
