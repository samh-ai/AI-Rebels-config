...
- **SageAttention 2.x is NOT on PyPI** — `pip install sageattention==2.2.0` will fail (only 1.x is on PyPI). Must build from source: clone `https://github.com/thu-ml/SageAttention.git`, then `pip install <dir> --no-build-isolation` using plain `pip` (the venv inherits system site-packages so system pip is visible to ComfyUI). Set `EXT_PARALLEL=4 NVCC_APPEND_FLAGS="--threads 8" MAX_JOBS=32` before building to speed up CUDA kernel compilation
